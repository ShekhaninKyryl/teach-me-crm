...
export class TutorsService {
                ...
                tutorProfile: {
                    create: {
                        ...
                        formats: dto.formats?.length
                            ? { create: dto.formats.map((f) => ({ format: f })) }
                            : undefined,
                    },
                },
            },
            include: {
                tutorProfile: { include: tutorInclude() },
            },
        });
    }
        ...
        if (dto.subjects) {
            const subjects = await Promise.all(
                dto.subjects.map((label) =>
                    this.prisma.subject.upsert({ where: { label }, update: {}, create: { label } }),
                ),
            );
    ...
    async saveTutorsStudents(tutorId: string, students: StudentAsUserDto[]): Promise<void> {
